<template>
  <ul v-if="posts.length > 0" class="">
    <li v-for="(post, index) in posts" :key="index" class="mb-4">
      <nuxt-link :to="`${postType}/${post.slug}`" class="card--clickable">
        ...
        <template v-else>
          <div class="popular-posts-item">
            ...
            <div class="popular-posts-item-content">
              ...
              <p class="mt-2">{{ post.description }}</p>
              <a href="#" class="posts-date" v-if="post.createdAt"
                ><i class="far fa-calendar-alt"></i> {{ formatDate(post.createdAt) }}</a
              >
            </div>
          </div>
        </template>
